  ...
  <div>
    <div class="has-text-centered">
      ...
      <button
        ...
        @click="view_grading_guide = !view_grading_guide"
      >
        View Grading Rule
      </button>
    </div>
    <section class="section is-small">
      <div class="container">
        <div
          class="box"
          v-for="(feedback, index) in all_feedbacks"
          :key="index"
        >
          <div class="has-text-centered">
            <h3 class="title is-3">
              <i> {{ feedback.username }}'s Feedback </i>
            </h3>
            <br />
          </div>
          <div class="columns">
              ...
              <div class="box has-background-link">
                <div class="box">
                  <div class="content">
                    ...
                    <ul>
                      <li
                        v-for="(answer, question, index) in feedback.answers"
                        :key="index"
                      >
                        <strong
                          ><u>{{ question }}</u></strong
        ...
        >
          <a
            ...
            v-if="previous_page"
            @click="get_info(previous_page)"
            >Previous</a
          >
          <a
            class="pagination-next"
            v-if="next_page"
            @click="get_info(next_page)"
            >Next</a
          >
        </nav>
      </div>
        ...
        <div class="box has-text-centered">
          ...
          <ul>
            ...
            <li v-for="(grading, key) in grading_rule" :key="key">
              Score: <u>{{ grading[0] }}</u> for
              <strong>"{{ grading[1] }}"</strong>
            </li>
            <li>Max Grade: {{ maxScore }}</li>
          </ul>
        </div>
      </div>
      <button
